...
# X_train, X_test, Y_train, Y_test = get_data(file_test)
...
# torch.save(model.state_dict(), PATH_3)  # 保存模型的状态字典
# 首先实例化模型的类对象
...
model3 = CNN()
model3.load_state_dict(torch.load(PATH_3))
normal = torch.tensor([0, 1])
anomalous = torch.tensor([1, 0])
for i, data in enumerate(test_data):
    ...
    for j in range(batch_size):
        ...
        if outputs[j][0] > outputs[j][1]:
            ...
            OUT.append(0)
        if outputs2[j][0] > outputs2[j][1]:
            OUT.append(1)
        else:
            OUT.append(0)
        if outputs3[j][0] > outputs3[j][1]:
            OUT.append(1)
        else:
            OUT.append(0)
        print(OUT, label[j][0].item())
        if find(OUT) == label[j][0].item():
            a = a + 1

print('正确率 ： ', a / len(test_data) / batch_size)
test(test_data)
